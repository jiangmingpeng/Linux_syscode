...
namespace ThreadMould
{
    ...
    //--------------上面的繁琐直接复制了
    using callback_t = std::function<void()>; // 一个用来保存无返回值无参数的包装器

    class Thread
    {
    public:
        // 初始化一个线程类 传入要调的函数
        Thread(callback_t cb) : _tid(-1), _status(THREAD_NEW), _cb(cb), _joinable(true)
        {
        ...
        void Die()
        {
            ...
            LOG(LogLevel::INFO)<<"The pthread was cancel";
        }
        ...
        void Detach()
        {
            ...
            LOG(LogLevel::INFO)<<"The pthread was detach";
        ...
        void Join()
        {
            if (_joinable)
            {
                ...
                ToStop();
                LOG(LogLevel::INFO)<<_name<<" The pthread was join....";
            }
            else
            {
    ...
    private:
        ...
        {
            Thread *Self = static_cast<Thread *>(self); // 强转类型效果只有一行 要想在域内使用 可以转成目标类型后保存给后续使用
            Self->_tid = pthread_self();
            pthread_setname_np(Self->_tid, Self->_name.c_str());
            Self->_cb();//唯一感觉有点技术含量的地方 这个回调和这个this指针的运用 
            Self->ToStop();
            return nullptr;
        }
        void ToStop()
